...
def fine_to_course(D, course_res):
    ...
    assert D.shape == (v_dim, s_dim,
                       u_dim), 'Input D has wrong input shape in function \'fine_to_course\'.'
    assert course_res[0] < v_dim, 'v-dimension of course_res larger than v_dim in function \'fine_to_course\'.'
    assert course_res[1] < u_dim, 'u-dimension of course_res larger than u_dim in function \'fine_to_course\'.'

    u_scale = course_res[1] / u_dim
    assert u_scale <= 1, 'The scaling factor of u-dimension is larger 1 in function \'fine_to_course\'.'

    D_ = resize(D, (course_res[0], s_dim, course_res[1]), order=0, mode='edge')
    D_ *= u_scale
...
def course_to_fine(D_epi_fine, D_epi_course):
    ...
    # Check dimensions of input data
    assert D_epi_fine.shape == (v_dim_fine, s_dim,
                                u_dim_fine), 'Input D_epi_fine has wrong input shape in function \'course_to_fine\'.'
    assert D_epi_course.shape == (v_dim_course, s_dim,
                                  u_dim_course), 'Input D_epi_course has wrong input shape in function \'course_to_fine\'.'
    assert v_dim_course < v_dim_fine, 'v-dimension of course image larger than v_dim of fine image in function \'course_to_fine\'.'
    assert u_dim_course < u_dim_fine, 'u-dimension of course image larger than v_dim of fine image in function \'course_to_fine\'.'

    u_scale = u_dim_course / u_dim_fine
    assert u_scale <= 1, 'The scaling factor of u-dimension is larger 1 in function \'course_to_fine\'.'

    D_tmp = np.zeros((v_dim_fine, s_dim, u_dim_fine), dtype=np.float64)
